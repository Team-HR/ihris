...
elseif (isset($_POST["updateBudget"])) {
  ...
  $sql = "UPDATE `ldpinhousetrainings` SET `budget` = '$budget' WHERE `ldpinhousetrainings_id` = '$ldpinhousetrainings_id'";
  $mysqli->query($sql);
  echo json_encode($b);
}
// update budget end
...
elseif (isset($_POST["getTrainings"])) {
  ...
  $sql = "INSERT INTO `ldpinhousetrainings` (`ldpinhousetrainings_id`, `ldplan_id`, `training_id`, `goal`, `numHours`, `participants`,`activities`, `evaluation`, `frequency`, `budgetReq`,`partner`,`budget`,`venue`) VALUES (NULL, '$ldplan_id', '$training_id', '$goal', '$numHours', '$participants', '$activities', '$evaluation', '$frequency', '$budgetReq', '$partner', '$budget', '$venue')";
