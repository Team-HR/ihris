    ...
    foreach ($cols as $i => $col) {
        if ($i === 0) {
            ...
            if ($col != "B") {
                ...
                if (!empty(trim($value["functional"]))) {
                    $functional = " (" . $value["functional"] . ")";
                } else {
                    $functional = "";
                }
                $spreadsheet->getActiveSheet()->setCellValue($col . ($aRow + $no), $value[$fields[$i]] . $functional)->getStyle($col . ($aRow + $no))->getAlignment()->setHorizontal('center')->setVertical('center')->setWrapText(true);
                $spreadsheet->getActiveSheet()->getStyle($col . ($aRow + $no))->getBorders()->getOutline()->setBorderStyle('thin');
            }
        }
    }
