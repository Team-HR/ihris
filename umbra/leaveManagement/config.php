...
if (isset($_POST['getEmployee'])) {
    ...
    if ($row = $lm_earning_query->fetch_assoc()) {
        $lm_earning_result = $row;
    }
    ...
    $sql = "SELECT `earning_id` FROM `lm_earning` WHERE `emp_id` = '$emp_id'";
    $res = $mysqli->query($sql);

    if ($row = $res->fetch_assoc()) {
        $res = $mysqli->query("UPDATE `lm_earning` SET `month_earned` = CURRENT_DATE, `vl_bal`='$vl_bal',`sl_bal`='$sl_bal' WHERE `earning_id` = '$row[earning_id]'");
        echo json_encode($res);
    } else {
        $mysqli->query("INSERT INTO `lm_earning`(`earning_id`, `emp_id`, `month_earned`, `vl_bal`, `sl_bal`) VALUES (NULL,'$emp_id','CURRENT_DATE','$vl_bal','$sl_bal')");
        echo json_encode($res);
    }
