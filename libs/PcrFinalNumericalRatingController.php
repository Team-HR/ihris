...
class PcrFinalNumericalRatingController extends Controller
{
    ...
    public function getFinalNumericalRating($employee_id = null,$period_id = null)
    {
        ...
        $total_weighted_average_core = number_format($total_weighted_average_core, "1", ".", ""); // debatable to either 1 decimal places or 2?
        $total_weighted_average_support = number_format($total_weighted_average_support, "2", ".", "");
        $final_numerical_rating = $total_weighted_average_core + $total_weighted_average_support;
        $final_numerical_rating = number_format($final_numerical_rating, "2", ".", "");
    ...
    private function compute_weighted_average($q, $e, $t, $percent, $disable)
    {
        ...
        if (!$count) return 0;
        $ave = $sum / $count;
        $weighted_average = $ave * intval($percent) / 100;
