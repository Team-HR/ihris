...
function load(year){
  $("#tableBody").load('applicantsProfile_proc.php',{
    load: true, 
    yearState: year
  } ,
    function(){
...
function addModalFunc(){
  ...
  $("#addNewForm").form({
        ...
        onSuccess: function(e){
          ...
          data = getInputValues();
          // post
          $.post('applicantsProfile_proc.php', {
            addNew: true,
            data0: data[0],
            data1: data[1]
...
function getValues(id){
  ...
  $.ajax({
    url: 'applicantsProfile_proc.php',
    async: false, 
    type: 'POST',
    dataType: 'json',
    data: {
...
function setInputValues(dataArr){
  ...
  $("input[name='sg']").val(dataArr[1]);
  // $("input[name='office']").val(dataArr[2]);
  $("#officeDropdown").dropdown("set selected",dataArr[2]),
  $("input[name='dateVacated']").val(dataArr[3]);
  
  $.each(objArray, function(index, val) {
     /* iterate through array or object */
     if (arr = dataArr[index+4]) {
      objArray[objArray[index]] = arr;
     } else if (dataArr[index+4] === null){
...
<div class="ui containerA" style="padding-left: 20px; padding-right: 20px;">
  <div class="ui borderless blue inverted mini menu">
    <div class="left item" style="margin-right: 0px !important;">
      <button onclick="window.location.href = 'index.php';" class="blue ui icon button" title="Back" style="width: 65px;">
        <i class="icon chevron left"></i> Back
      </button>
    </div>
    <div class="item">
     <h3> <i class="users icon"></i> Applicants' Profile
          <i class="caret right icon"></i>
          <i class="briefcase icon"></i> Vacant Positions
        </h3>
   </div>
