...
while ($row = $result->fetch_assoc()) {
	$sql2 = "SELECT * FROM  `setup_salary_adjustments` WHERE `schedule` = '$row[salary_schedule]' and active = '1'";
			$sql2 = $mysqli->query($sql2);
			$sql2 = $sql2 ->fetch_assoc();

			$sql3 = "SELECT * FROM  `setup_salary_adjustments_setup` WHERE `parent_id` = '$sql2[id]' AND `position`='$row[education]'";
...
elseif (isset($_POST["addPlantilla"])) {
	...
	$sql = "INSERT INTO `plantillas_test` (`id`, `item_no`, `page_no`, `department_id`, `position_title`, `functional_title`, `level`, `sg`, `authorized_salary`, `actual_salary`, `step`, `area_code`, `area_type`, `category`, `incumbent`, `reason_of_vacancy`, `other`,`vacated_by`, `original_appointment`, `last_promotion`, `supervisor`, `abolish`, `timestamp_created`, `timestamp_updated`) VALUES ('$id', '$item_no', '$page_no', '$department', '$position',  '$functional_title', '$level', '$sg', NULL, NULL, '$step', NULL, NULL, NULL, '$incumbent', '$reason_of_vacancy', '$other', '$vacated_by', '$original_appointment', '$last_promotion','$supervisor', '$abolish', NULL, NULL)";
		$mysqli->query($sql);
